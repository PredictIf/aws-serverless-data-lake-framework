...
function send_legislators()
{
  ...
  do
    ...
    if [ "$CENTRAL_BUCKET" == "$STAGE_BUCKET" ];then
      aws s3 cp $ORIGIN$FILE "${S3_DESTINATION}raw/engineering/legislators/$table/" --profile $PROFILE --sse aws:kms --sse-kms-key-id $KMS_KEY
    else
      aws s3 cp $ORIGIN$FILE "${S3_DESTINATION}engineering/legislators/$table/" --profile $PROFILE --sse aws:kms --sse-kms-key-id $KMS_KEY
    fi
